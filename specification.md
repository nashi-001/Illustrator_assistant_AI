...
```
return_struct1 = illust_assistant_ai.最大レベル物体抽出(画像、絶対条件、もしあるなら過去データ);
...
return_adjust1 = defalt_client.adjust(return_struct)
...
return_struct2 = illust_assistant_ai.中級レベル物体抽出(return_adjust1から色塗りレイヤーを除いたもの)
...
```
上記のコードは`return_struct`や`return_adjust`は人間や背景、建物を検出し、形を整えてまた渡す...といったことを繰り返す関数群です。<br>
